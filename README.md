# Kifiya Auth - Payment Verification System

## Overview

Kifiya Auth is an instant payment verification system designed for Ethiopian businesses to automate payment verification across multiple banks. The platform supports two user types: normal merchants (vendors) who use the dashboard interface, and developers who integrate via API.

**Supported Banks**: BOA (Bank of Abyssinia), Awash Bank, CBE (Commercial Bank of Ethiopia), Telebirr, and more

## Documentation

This repository contains comprehensive documentation for the Kifiya Auth project:

- **[Kifiya Auth Overview](./kifiya-auth-overview.md)** - What Kifiya Auth is and what it includes (non-technical overview)
- **[PayAuth Analysis](./pay-auth.md)** - Analysis of PayAuth (competitor/inspiration)
- **[Kifiya Features](./kifiya-feature.md)** - Complete feature list with technical details
...
## What Kifiya Auth Does

Kifiya Auth **verifies** that bank transfers have been completed across multiple Ethiopian banks. It does **NOT**:
...
Kifiya Auth validates payments in two ways:
...
```javascript
const response = await fetch("https://api.kifiya-auth.com/v1/verifications", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
...
```javascript
const response = await fetch(
  "https://api.kifiya-auth.com/v1/verifications/qr",
  {
    method: "POST",
    headers: {
      Authorization: "Bearer YOUR_API_KEY",
...
Kifiya Auth is designed for two types of users:
...
- **[Overview](./kifiya-auth-overview.md)** - Start here to understand what Kifiya Auth is
