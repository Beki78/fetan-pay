...
export async function verifyAbyssinia(
  ...
  try {
    logger.info(
      `🏦 Starting Abyssinia verification for reference: ${reference} with suffix: ${suffix}`,
    );
    ...
    logger.info(
      `✅ Successfully fetched response with status: ${response.status}`,
    );
    ...
    logger.debug(
      `🔍 Available fields in transaction data:`,
      Object.keys(transactionData),
    );
    logger.debug(
      `📊 Number of fields in transaction: ${Object.keys(transactionData).length}`,
    );
    ...
    logger.debug(
      `   payer: "${transactionData["Payer's Name"]}" -> "${result.payer}"`,
    );
    logger.debug(
      `   payerAccount: "${transactionData['Source Account']}" -> "${result.payerAccount}"`,
    );
    logger.debug(
      `   receiver: "${transactionData['Source Account Name']}" -> "${result.receiver}"`,
    );
    logger.debug(
      `   amount: "${transactionData['Transferred Amount']}" -> ${result.amount}`,
    );
    logger.debug(
      `   date: "${transactionData['Transaction Date']}" -> ${result.date}`,
    );
    logger.debug(
      `   reference: "${transactionData['Transaction Reference']}" -> "${result.reference}"`,
    );
    logger.debug(
      `   reason: "${transactionData['Narrative']}" -> "${result.reason}"`,
    );

    logger.debug(
      `✅ Field mapping completed. Mapped ${Object.keys(result).length} fields.`,
    );

    logger.debug(
      `📋 Final mapped result object:`,
      JSON.stringify(result, null, 2),
    );
    logger.info(
      `✅ Successfully parsed Abyssinia receipt for reference: ${result.reference}`,
    );
    logger.debug(
      `💰 Key transaction details - Amount: ${result.amount}, Payer: ${result.payer}, Date: ${result.date}`,
    );
