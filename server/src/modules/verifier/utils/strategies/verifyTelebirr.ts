  ...
  const getSettledAmount = (): string => {
    ...
    let amount = $('td.receipttableTd.receipttableTd2')
      .filter((_, el) => {
        ...
        return (
          prevTd.text().includes('የተከፈለው መጠን') ||
          prevTd.text().includes('Settled Amount')
        );
      })
      .text()
      .trim();
...
async function fetchFromPrimarySource(
  ...
  try {
    logger.info(
      `Attempting to fetch Telebirr receipt from primary source: ${url}`,
    );
    ...
    logger.info(
      `Successfully extracted Telebirr data for reference: ${reference}`,
      {
    ...
    logger.error(
      `Error fetching Telebirr receipt from primary source ${url}:`,
      {
        error: errorMessage,
        stack: errorStack,
        ...responseDetails,
  ...
  try {
    ...
    if (typeof data === 'string') {
      try {
        ...
        logger.warn(
          'Proxy response is not valid JSON, attempting to scrape as HTML',
        );
    ...
    if (!extractedData) {
      logger.warn(
        'Failed to parse JSON from proxy, attempting to scrape as HTML',
      );
    ...
    logger.info(
      `Successfully extracted Telebirr data from proxy for reference: ${reference}`,
      {
        receiptNo: extractedData.receiptNo,
        payerName: extractedData.payerName,
        transactionStatus: extractedData.transactionStatus,
...
export async function verifyTelebirr(
  ...
  const cacheKey = verificationCache.getKey('TELEBIRR', reference);

  // Check cache first
  const cached = verificationCache.get<TelebirrReceipt>(cacheKey);
  if (cached && isValidReceipt(cached)) {
    logger.info(`✅ Cache hit for Telebirr verification: ${reference}`);
    return cached;
  }
  ...
  const result = await retryWithBackoff(
    async () => {
      ...
      if (!skipPrimary) {
        ...
        logger.warn(
          `Primary Telebirr verification failed for reference: ${reference}. Trying fallback proxy...`,
        );
      } else {
        logger.info(
          `Skipping primary verifier due to SKIP_PRIMARY_VERIFICATION=true`,
        );
      }
      ...
      if (fallbackResult && isValidReceipt(fallbackResult)) {
        logger.info(
          `Successfully verified Telebirr receipt using fallback proxy for reference: ${reference}`,
        );
        return fallbackResult;
      }

      throw new Error(
        `Both primary and fallback Telebirr verification failed for reference: ${reference}`,
      );
    },
    {
      maxRetries: 2,
  ...
  if (result && isValidReceipt(result)) {
    ...
    logger.info(`✅ Cached Telebirr verification result: ${reference}`);
  }
...
function isValidReceipt(receipt: TelebirrReceipt): boolean {
  ...
  return Boolean(
    receipt.receiptNo && receipt.payerName && receipt.transactionStatus,
  );
}
