...
async function parseDashenReceipt(
  ...
  try {
    ...
    logger.debug(
      `👤 Sender name regex result: ${senderNameMatch ? `Found: "${senderName}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `🏦 Sender account regex result: ${senderAccountMatch ? `Found: "${senderAccountNumber}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `💳 Transaction channel regex result: ${transactionChannelMatch ? `Found: "${transactionChannel}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `🔧 Service type regex result: ${serviceTypeMatch ? `Found: "${serviceType}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `📝 Narrative regex result: ${narrativeMatch ? `Found: "${narrative}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `📞 Receiver name regex result: ${receiverNameMatch ? `Found: "${receiverName}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `📱 Phone number regex result: ${phoneNoMatch ? `Found: "${phoneNo}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `🏢 Institution name regex result: ${institutionNameMatch ? `Found: "${institutionName}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `🔢 Transaction reference regex result: ${transactionReferenceMatch ? `Found: "${transactionReference}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `🔄 Transfer reference regex result: ${transferReferenceMatch ? `Found: "${transferReference}"` : 'No match'}`,
    );
    ...
    logger.debug(
      `📅 Date regex result: ${dateMatch ? `Found: "${dateRaw}"` : 'No match'}`,
    );
    ...
    const transactionAmount = extractAmountWithLogging(
      rawText,
      /Transaction\s*Amount\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Transaction Amount',
    );
    const serviceCharge = extractAmountWithLogging(
      rawText,
      /Service\s*Charge\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Service Charge',
    );
    const exciseTax = extractAmountWithLogging(
      rawText,
      /Excise\s*Tax\s*(?:\(15%\))?\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Excise Tax',
    );
    const vat = extractAmountWithLogging(
      rawText,
      /VAT\s*(?:\(15%\))?\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'VAT',
    );
    const penaltyFee = extractAmountWithLogging(
      rawText,
      /Penalty\s*Fee\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Penalty Fee',
    );
    const incomeTaxFee = extractAmountWithLogging(
      rawText,
      /Income\s*Tax\s*Fee\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Income Tax Fee',
    );
    const interestFee = extractAmountWithLogging(
      rawText,
      /Interest\s*Fee\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Interest Fee',
    );
    const stampDuty = extractAmountWithLogging(
      rawText,
      /Stamp\s*Duty\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Stamp Duty',
    );
    const discountAmount = extractAmountWithLogging(
      rawText,
      /Discount\s*Amount\s*(?:ETB|Birr)?\s*([\d,]+\.?\d*)/i,
      'Discount Amount',
    );
    const total = extractAmountWithLogging(
      rawText,
    ...
    const formattedReceiverName = receiverName
      ? titleCase(receiverName)
      : undefined;
    const formattedInstitutionName = institutionName
      ? titleCase(institutionName)
      : undefined;

    logger.debug(
      `✨ Formatted names - Sender: "${formattedSenderName}", Receiver: "${formattedReceiverName}", Institution: "${formattedInstitutionName}"`,
    );
    ...
    logger.debug(
      `🔍 Validation check - Transaction Reference: ${transactionReference ? '✅' : '❌'}, Transaction Amount: ${transactionAmount ? '✅' : '❌'}`,
    );
    if (transactionReference && transactionAmount) {
      ...
      logger.warn(
        `❌ Missing fields: ${!transactionReference ? 'Transaction Reference ' : ''}${!transactionAmount ? 'Transaction Amount' : ''}`,
      );
      return {
        success: false,
        error:
          'Could not extract required fields (Transaction Reference and Amount) from PDF.',
      };
    }
  } catch (parseErr: any) {
...
function extractAmountWithLogging(
  text: string,
  regex: RegExp,
  fieldName: string,
): number | undefined {
  ...
  if (match && match[1]) {
    ...
    logger.debug(
      `💰 ${fieldName} regex result: Found: "${rawValue}" → Parsed: ${result}`,
    );
    return result;
  } else {
