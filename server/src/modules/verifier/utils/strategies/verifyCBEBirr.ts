...
export async function verifyCBEBirr(
  ...
  try {
    logger.info(
      `[CBEBirr] Starting verification for receipt: ${receiptNumber}, phone: ${phoneNumber}`,
    );
...
function parseCBEBirrReceipt(pdfText: string): CBEBirrReceipt | null {
  try {
    ...
    const customerName =
      extractValue(pdfText, /Customer Name:\s*([^\n\r]+?)(?=\s*Region:)/i) ||
    ...
    const orderId =
      extractValue(pdfText, /Order ID[\s\n\r]+([A-Z0-9]+)/i) ||
