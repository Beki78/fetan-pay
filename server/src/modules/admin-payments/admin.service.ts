...
export class AdminPaymentsService {
  ...
  async listAllPayments(query: ListAllPaymentsDto, req: Request) {
    ...
    const paymentStatusFilter =
      query.status &&
      ['VERIFIED', 'UNVERIFIED', 'PENDING'].includes(query.status)
    ...
    const unifiedPayments: UnifiedPayment[] = standalonePayments.map(
      (payment: any) => {
        ...
        const otherBankName =
          (verificationPayload.otherBankName as string) || null;

        // Use custom bank name if available, otherwise use provider enum
        const displayProvider = otherBankName || payment.provider;
        ...
        return {
          ...
          receiverAccount: payment.receiverAccount?.receiverAccount || null,
