...
import {
  ...
  type ReceiverStatus,
  type WalletDepositReceiverAccount,
...
const STATUS_OPTIONS: ReceiverStatus[] = ["ACTIVE", "INACTIVE"];

export default function WalletReceiversPage() {
  const { data: receivers, isLoading, error } = useGetDepositReceiversQuery();
  const [createReceiver, { isLoading: isCreating }] = useSetDepositReceiverMutation();
  const [updateReceiver, { isLoading: isUpdating }] = useUpdateDepositReceiverMutation();
  const [deleteReceiver, { isLoading: isDeleting }] = useDeleteDepositReceiverMutation();
  ...
  const [status, setStatus] = useState<ReceiverStatus>("ACTIVE");
  ...
  const errorMessage = useMemo(() => {
    ...
    return anyErr?.data?.message ?? "Failed to load deposit receivers";
  }, [error]);
  ...
  const handleEdit = (receiver: WalletDepositReceiverAccount) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setMessage({ type: "success", text: "Receiver account updated successfully" });
      } else {
        await createReceiver(data).unwrap();
        setMessage({ type: "success", text: "Receiver account created successfully" });
      }
      resetForm();
    } catch (e: any) {
      setMessage({ type: "error", text: e?.data?.message ?? "Failed to save receiver account" });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setMessage({ type: "success", text: "Receiver account deleted successfully" });
      setDeleteConfirmId(null);
    } catch (e: any) {
      setMessage({ type: "error", text: e?.data?.message ?? "Failed to delete receiver account" });
    }
  };

  const statusBadge = (s: ReceiverStatus) => {
    return s === "ACTIVE" ? (
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-1">
          Wallet Deposit Receivers
        </h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Configure accounts where merchants send money for wallet top-ups. Merchants will see these accounts when they want to deposit funds.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800/50">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">
          {editingId ? "Update Receiver Account" : "Add Receiver Account"}
        </h2>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div>
            ...
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as ReceiverStatus)}
              className="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800"
            >
          ...
          <div>
            ...
            <Input
              ...
              placeholder="e.g., FetanPay Wallet Deposits"
            />
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            <Input
              ...
              placeholder="e.g., FetanPay Wallet Deposit - CBE"
            />
      ...
      <div className="rounded-xl bg-gray-50 dark:bg-gray-800/30 p-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          These accounts will be visible to merchants for wallet top-ups.
        </p>
        ...
        <div className="bg-white dark:bg-gray-800/50 rounded-lg overflow-hidden border-0">
          {isLoading ? (
            ...
            <div className="px-4 py-6 text-sm text-gray-500 dark:text-gray-400">
              No receiver accounts configured yet.
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 max-w-md w-full mx-4">
            ...
            <p className="text-gray-700 dark:text-gray-300 mb-4">
              Are you sure you want to delete this receiver account? This action cannot be undone.
            </p>
